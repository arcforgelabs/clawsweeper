...
- Confirm the issue still fits the target repository direction source before
  editing: read INTENT.md first when present, otherwise VISION.md. When both
  exist, INTENT.md is primary.
- Keep the implementation small and direct; stop if the work becomes medium or
  ...
  with the selected direction guardrails.
- Prefer existing plugin, ClawHub, extension, config, and docs surfaces when
  INTENT.md/VISION.md says optional capability should live outside core.
- The final PR must stay narrow and should carry the \`${AUTOGENERATED_LABEL}\`
  label; the executor also applies this label after opening or updating the PR.
`
    : "";
