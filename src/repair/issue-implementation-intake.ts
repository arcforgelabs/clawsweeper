...
function visionFitImplementationPrompt(context: LooseRecord) {
  ...
  return [
    ...
    "",
    "- read INTENT.md first when present, otherwise VISION.md, before editing",
    "- when both files exist, treat INTENT.md as the primary direction source",
    "- stop if the issue no longer fits the selected direction source",
    "- keep one small focused PR",
    "- stop if the work expands into medium/large scope or needs a product decision",
    "- preserve plugin, ClawHub, extension, config, and docs boundaries from INTENT.md/VISION.md",
    "",
    "Review work prompt:",
    "",
