...
security/protected signal, no product-decision blocker, and complete
direction-source evidence plus repair shape. Reviews use `INTENT.md` first when
present and fall back to `VISION.md`. This lane is for small direction-aligned
issue work; medium or broad aligned work stays manual.
...
re-read `INTENT.md` when present, otherwise `VISION.md`, keep the PR small, and
stop when the work no longer fits the selected direction guardrails. Any PR
created from the job is labeled
