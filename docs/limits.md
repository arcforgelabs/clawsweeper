...
| --- | ---: | --- |
| `workers.max` | 2 | Maximum global Codex worker budget used to derive lane limits. |
...
`workers.max = 2`, normal review can use 1 worker, hot intake can use 1,
...
| `review_shards.normal_default` | 1 | Quiet-system normal review shard ceiling. |
...
| `review_shards.hard_cap` | 2 | Maximum accepted review shard count. |
| `commit_review.page_size_default` | 1 | Commits selected per commit-review page. |
| `commit_review.page_size_hard_cap` | 2 | Maximum commit-review page size. |
| `repair_live_runs.default` | 1 | Default live repair workflow run cap for manual dispatch/requeue/self-heal. |
| `repair_live_runs.hard_cap` | 2 | Absolute live repair run cap accepted by the CLI. |
...
- Quiet system: manual normal review can request 1 shard; scheduled normal
...
- 1 active repair worker and 1 active background worker: normal review gets
  1 because `2 - 1 interactive reserve - 1 expansion reserve - 1 priority
